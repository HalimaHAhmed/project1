...
def create_folder(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            folder = Folder.objects.create(name=folder_name, user=folder_owner)
            folder.save()
...
def folder_detail(request, pk):
    ...
    return render(request, 'folder_detail.html', context={'folder': folder, 'files': files})
def upload_file(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('folder_detail', pk)
        else:
            messages.error(request, 'Please fill all the required fields')
    form = FileForm()
